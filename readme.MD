...
* Delete existing service account (These 2 steps are used only when there is existing service accounts)
```
...
* This command is used to destroy folders one by one in bash

```
for dir in 80-cdn 70-ecr 20-bastion; do
  cd $dir
